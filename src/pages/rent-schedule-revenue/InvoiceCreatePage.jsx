...
export default function InvoiceCreatePage() {
  ...
  const { subtotal, taxAmount, totalAmount } = lineItems.reduce(
    (acc, row) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.agreement) {
      toast.error("Select an agreement");
      return;
    }
    ...
    try {
      const lineItemsPayload = validItems.map((r) => {
        const { amount } = computeLineAmount(r);
        return {
  ...
  return (
    <div>
      <PageHeader title="Create Invoice" backTo={BASE_PATH} />
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {lineItems.map((row, idx) => {
                  const { amount, total } = computeLineAmount(row);
                  return (
                    <tr key={idx} className="border-b border-gray-100">
                      <td className="py-2">
                        <input
                          ...
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                        />
                      </td>
                      <td className="py-2">
                        <input
                          ...
                          onChange={setLineItem(idx, "period_start")}
                      ...
                      <td className="py-2">
                        <input
                          ...
                          onChange={setLineItem(idx, "period_end")}
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-2 focus:ring-emerald-500"
                        />
                      </td>
                      <td className="py-2 text-right">
                        <input
                          ...
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm text-right focus:ring-2 focus:ring-emerald-500"
                        />
                      </td>
                      <td className="py-2 text-right">
                        <input
                      ...
                      <td className="py-2 text-right">
                        <input
                          ...
                          step="0.01"
                          value={row.tax_rate}
                          onChange={setLineItem(idx, "tax_rate")}
                          className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm text-right focus:ring-2 focus:ring-emerald-500"
                        />
                      </td>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 flex justify-end">
            <div className="text-sm space-y-1 text-right">
              ...
              <p className="text-base font-semibold">
                Total: <span>₹{totalAmountFinal.toFixed(2)}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 pt-4">
          <Button variant="secondary" type="button" onClick={() => navigate(BASE_PATH)}>
            Cancel
          </Button>
