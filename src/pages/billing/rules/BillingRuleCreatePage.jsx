...
export default function BillingRuleCreatePage({ inModal = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isPercentage(form.calculation_method)) {
        payload.rate = parseFloat(form.rate) || null;
      } else {
        payload.amount = parseFloat(form.amount) || null;
      }
      if (form.max_cap_amount) payload.max_cap_amount = parseFloat(form.max_cap_amount);
      const res = await billingRulesAPI.create(payload);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card className={inModal ? "p-0 border-0 shadow-none" : "p-6 max-w-xl"}>
        <div className="space-y-4">
          <Input label="Name" value={form.name} onChange={set("name")} required placeholder="e.g. Late Payment Fee 2%" />
          <div>
