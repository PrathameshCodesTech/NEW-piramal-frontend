...
export default function FloorEditPage() {
  ...
  useEffect(() => {
    floorsAPI.get(id).then((res) => {
      ...
      setForm({
        ...
        cam_area_sqft: res.cam_area_sqft ?? "",
        floor_type: res.floor_type || "",
        allowed_use: res.allowed_use || "",
        leasing_type: res.leasing_type || "",
        max_units_allowed: res.max_units_allowed ?? "",
      });
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await floorsAPI.update(id, payload);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="border-l-2 border-emerald-500 pl-5 py-5 pr-5 rounded-r-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Input label="Label" icon={Tag} value={form.label} onChange={set("label")} />
            <Select label="Status" value={form.status} onChange={set("status")} options={FLOOR_STATUS} />
            <Input label="Floor Type" icon={LayoutGrid} value={form.floor_type} onChange={set("floor_type")} />
          </div>
        </div>
