...
export default function SiteEditPage() {
  ...
  useEffect(() => {
    sitesAPI.get(id).then((res) => {
      setForm({
        name: res.name || "",
        code: res.code || "",
        site_type: res.site_type || "COMMERCIAL",
        ...
        country: res.country || "",
        pincode: res.pincode || "",
        total_builtup_area_sqft: res.total_builtup_area_sqft ?? "",
        leasable_area_sqft: res.leasable_area_sqft ?? "",
        common_area_percent: res.common_area_percent ?? "",
        common_area_sqft: res.common_area_sqft ?? "",
        base_rate_sqft: res.base_rate_sqft ?? "",
        ownership_type: res.ownership_type || "OWN",
        management_fee_type: res.management_fee_type || "PERCENTAGE",
        management_fee_value: res.management_fee_value ?? "",
        contract_start_date: res.contract_start_date || "",
        contract_end_date: res.contract_end_date || "",
  ...
  return (
    <div>
      <PageHeader title="Edit Site" backTo={`/properties/sites/${id}`} />
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-2 pt-4">
          <Button variant="secondary" type="button" onClick={() => navigate(`/properties/sites/${id}`)}>Cancel</Button>
