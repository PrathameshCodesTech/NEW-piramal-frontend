...
export default function EscalationTemplateEditPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    name: "", description: "", escalation_type: "FIXED_PERCENT",
    frequency: "ANNUAL", escalation_percentage: "", first_escalation_months: "12",
    index_name: "", index_base_value: "", step_schedule_text: "", rounding_rule: "NEAREST_DOLLAR",
    applicability: "ALL", status: "DRAFT", cap_percentage: "", floor_percentage: "",
    apply_to_cam: false, apply_to_parking: false,
  });
  ...
  useEffect(() => {
    setLoading(true);
    escalationTemplatesAPI.get(id).then((res) => {
      setForm({
        name: res?.name || "", description: res?.description || "",
        escalation_type: res?.escalation_type || "FIXED_PERCENT",
        frequency: res?.frequency || "ANNUAL",
        escalation_percentage: res?.escalation_percentage ?? "",
        first_escalation_months: res?.first_escalation_months ?? "12",
        index_name: res?.index_name || "",
        index_base_value: res?.index_base_value ?? "",
        step_schedule_text: Array.isArray(res?.step_schedule) && res.step_schedule.length
          ? JSON.stringify(res.step_schedule, null, 2)
          : "",
        rounding_rule: res?.rounding_rule || "NEAREST_DOLLAR",
        applicability: res?.applicability || "ALL",
        status: res?.status || "DRAFT",
        cap_percentage: res?.cap_percentage ?? "",
        floor_percentage: res?.floor_percentage ?? "",
        apply_to_cam: !!res?.apply_to_cam,
        apply_to_parking: !!res?.apply_to_parking,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await escalationTemplatesAPI.update(id, payload);
      toast.success("Template updated");
      navigate(`/leases/escalation-templates/${id}`);
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title="Edit Escalation Template" backTo={`/leases/escalation-templates/${id}`} />
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-2 pt-4">
          <Button variant="secondary" type="button" onClick={() => navigate(`/leases/escalation-templates/${id}`)}>Cancel</Button>
