...
export default function RoleCreatePage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    try { await rolesAPI.create(form); toast.success("Role created"); navigate("/admin/roles"); } catch (err) { toast.error(err.message); } finally { setLoading(false); }
  };

  return (
    <div>
      <PageHeader title="Create Role" backTo="/admin/roles" />
      <Card className="p-6 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="secondary" type="button" onClick={() => navigate("/admin/roles")}>Cancel</Button>
