...
export default function CompanyEditPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSaving(true);
    try { await companiesAPI.update(id, form); toast.success("Company updated"); navigate(`/admin/companies/${id}`); } catch (err) { toast.error(err.message); } finally { setSaving(false); }
  };
  ...
  return (
    <div>
      <PageHeader title="Edit Company" backTo={`/admin/companies/${id}`} />
      <Card className="p-6 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="secondary" type="button" onClick={() => navigate(`/admin/companies/${id}`)}>Cancel</Button>
