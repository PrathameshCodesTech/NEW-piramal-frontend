...
export default function EntityCreatePage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    try { await entitiesAPI.create(form); toast.success("Entity created"); navigate("/admin/entities"); } catch (err) { toast.error(err.message); } finally { setLoading(false); }
  };

  return (
    <div>
      <PageHeader title="Create Entity" backTo="/admin/entities" />
      <Card className="p-6 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="secondary" type="button" onClick={() => navigate("/admin/entities")}>Cancel</Button>
